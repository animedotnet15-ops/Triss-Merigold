...
# Defaults only - real values are read per-call from
# settings.shortener.anti_bypass.{strike_limit,mute_seconds} at the call
# sites (triss.handlers.start, triss.handlers.callbacks) so the owner's
# /settings -> Shortener -> Anti-Bypass configuration takes effect
# immediately, no restart required.
_DEFAULT_FAILURE_WINDOW_SECONDS = 600
_DEFAULT_FAILURE_THRESHOLD = 3
_failure_log: dict[int, list[float]] = {}


def record_failed_attempt(user_id: int, window_seconds: int = _DEFAULT_FAILURE_WINDOW_SECONDS) -> int:
    """Records one failed (bypass/invalid) attempt and returns how many
    attempts are still within `window_seconds` (including this one) - the
    caller compares this to the configured strike_limit to decide between
    showing bypass_message (with a "Warning {count}" line) or
    muted_message (see triss.handlers.start)."""
    now = time.time()
    attempts = [t for t in _failure_log.get(user_id, []) if now - t < window_seconds]
    attempts.append(now)
    _failure_log[user_id] = attempts
    return len(attempts)


def is_rate_limited(user_id: int, threshold: int = _DEFAULT_FAILURE_THRESHOLD,
                     window_seconds: int = _DEFAULT_FAILURE_WINDOW_SECONDS) -> bool:
    now = time.time()
    attempts = [t for t in _failure_log.get(user_id, []) if now - t < window_seconds]
    _failure_log[user_id] = attempts
...
async def generate_short_link(shortener_settings: dict, long_url: str) -> Optional[str]:
    ...
    if provider is None:
        ...
        return None
    # ROOT-CAUSE GUARD — a garbage long_url built from a missing/None value
    # must be caught here, at the source, with a diagnostic that says WHY —
    # not just forwarded to the provider to fail on with a generic message.
    # Nothing here logs secrets: long_url at this point is only ever a
