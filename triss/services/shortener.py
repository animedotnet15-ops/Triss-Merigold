...
All real verification happens server-side once Telegram receives the
`/start verify_<session_id><proof>` deep link — see `evaluate_verification()`
below. If PUBLIC_BASE_URL is configured, the shortener is pointed at the
bot's OWN branded "Verifying..." landing page first
(`{PUBLIC_BASE_URL}/v/<session_id>`, see triss.web.server) instead of the
Telegram deep link directly — that page is PURELY COSMETIC: it performs
no checks of its own and simply redirects into the same deep link after
a couple of seconds. Whether or not that landing page is used, the
security model below is identical — it is described here in terms of the
deep link, since that is the step that actually matters.
...
async def generate_short_link(shortener_settings: dict, long_url: str) -> Optional[str]:
    ...
    # https://t.me/... deep link, an https://<PUBLIC_BASE_URL>/v/... landing
    # link (cosmetic only - see triss.web.server module docstring), or
    # None/empty.
    parsed = urlparse(long_url) if long_url else None
    allowed_netlocs = {"t.me"}
    if config.public_base_url:
        allowed_netlocs.add(urlparse(config.public_base_url).netloc)
    url_valid = bool(parsed and parsed.scheme == "https" and parsed.netloc in allowed_netlocs)
    if not url_valid:
        logger.error(
            ...
            parsed.netloc if parsed else None,
        )
        return None
    try:
...
def build_verification_link(bot_username: str, session_id: str, raw_proof: str) -> str:
    """Returns the URL that gets shortened and shown to the user.

    If PUBLIC_BASE_URL is configured, this is the bot's OWN branded
    "Verifying..." landing page (`{PUBLIC_BASE_URL}/v/<session_id>?p=<proof>`
    - see triss.web.server.verify_landing) - purely cosmetic, the page
    itself performs no verification and simply redirects the browser to
    the real Telegram deep link below after a couple of seconds.

    If PUBLIC_BASE_URL is NOT configured, this returns the Telegram deep
    link directly (`build_verification_deep_link`), exactly as before the
    landing page existed - no functional difference to verification
    either way, since ALL real checks happen in evaluate_verification()
    once Telegram receives the /start, never in the landing page."""
    if config.public_base_url:
        return f"{config.public_base_url}/v/{session_id}?p={quote(raw_proof, safe='')}"
    return build_verification_deep_link(bot_username, session_id, raw_proof)
...
async def start_new_verification(bot_username: str, user_id: int, access_token: str,
    ...
    immediately, and shortens the verification link (see
    `build_verification_link` - either the bot's own branded landing page,
    or the Telegram deep link directly, depending on whether
    PUBLIC_BASE_URL is configured). Returns (session_doc, short_url);
    short_url is None if the provider call or configuration failed —
    callers must treat that as 'verification unavailable', never silently
    deliver the file."""
    ...
    # link. Catch it here, at the source, instead of downstream. Still
    # required even when routing through the landing page, since the
    # landing page's own client-side JS builds the same deep link from
    # bot_username to redirect into at the end.
    if not bot_username:
        logger.error(
    ...
    verification_link = build_verification_link(bot_username, session_id, raw_proof)
    short_url = await generate_short_link(shortener_settings, verification_link)
