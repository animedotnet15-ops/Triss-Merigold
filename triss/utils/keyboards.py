...
def settings_main_menu(settings: dict | None = None) -> InlineKeyboardMarkup:
    ...
    each toggle-style button shows its current 🟢/🔴 state at a glance,
    instead of the owner having to open each submenu to check."""
    s = settings or {}
    fs = " 🟢" if s.get("force_sub_enabled", True) else " 🔴"
    ad = " 🟢" if s.get("auto_delete", {}).get("enabled") else " 🔴"
    sh = " 🟢" if s.get("shortener", {}).get("enabled") else " 🔴"
    ld = " 🟢" if s.get("linkdl", {}).get("enabled") else " 🔴"
    ef = " 🟢" if s.get("effects", {}).get("master_enabled", True) else " 🔴"
    mt = " 🟢" if s.get("maintenance") else " 🔴"
    return InlineKeyboardMarkup([
        ...
        [btn(f"🌐 sʜᴏʀᴛᴇɴᴇʀ{sh}", "settings:shortener"), btn(f"🎬 ᴇғғᴇᴄᴛs{ef}", "settings:effects")],
        [btn(f"⬇️ ᴅɪʀᴇᴄᴛ ᴅᴏᴡɴʟᴏᴀᴅ (/ʟɪɴᴋᴅʟ){ld}", "settings:linkdl")],
...
def effects_main_menu(effects_settings: dict) -> InlineKeyboardMarkup:
    """Top of the consolidated 🎬 Effects settings — one button per
    category (each opens a multi-select emoji picker), plus a single
    master on/off that overrides every category at once without
    touching their individual configuration."""
    from triss.utils.effects import EFFECT_CATEGORIES
    master = effects_settings.get("master_enabled", True)
    categories = effects_settings.get("categories", {})
    rows = []
    row = []
    for key, label in EFFECT_CATEGORIES.items():
        cat = categories.get(key, {})
        tag = " 🟢" if cat.get("enabled") and cat.get("selected") else " 🔴"
        row.append(btn(f"{label}{tag}", f"eff:{key}:open"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    master_label = "🔘 All Effects: 🟢 ON" if master else "🔘 All Effects: 🔴 OFF"
    rows.append([btn(master_label, "eff:master", ButtonStyle.SUCCESS if master else ButtonStyle.DANGER)])
    rows.append([back_btn()])
    return InlineKeyboardMarkup(rows)


def effect_category_menu(category: str, category_settings: dict) -> InlineKeyboardMarkup:
    """Multi-select emoji grid for one effect category - each emoji
    toggles independently (✅ when selected, any number can be on at
    once), plus this category's own on/off switch."""
    from triss.utils.effects import EFFECT_LABELS
    enabled = category_settings.get("enabled", False)
    selected = set(category_settings.get("selected") or [])
    rows = []
    row = []
    for key, label in EFFECT_LABELS.items():
        tag = " ✅" if key in selected else ""
        row.append(btn(f"{label}{tag}", f"eff:{category}:pick:{key}"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    toggle_label = "This category: 🟢 ON" if enabled else "This category: 🔴 OFF"
    toggle_style = ButtonStyle.SUCCESS if enabled else ButtonStyle.DANGER
    rows.append([btn(toggle_label, f"eff:{category}:toggle", toggle_style)])
    rows.append([back_btn("settings:effects")])
    return InlineKeyboardMarkup(rows)
