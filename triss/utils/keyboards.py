...
def settings_main_menu() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [btn("⚡ sᴛᴀʀᴛ ᴇғғᴇᴄᴛ", "settings:starteffect"), btn("🚨 ʙʏᴘᴀss ᴇғғᴇᴄᴛ", "settings:bypasseffect")],
        [btn("⬇️ ᴅɪʀᴇᴄᴛ ᴅᴏᴡɴʟᴏᴀᴅ (/ʟɪɴᴋᴅʟ)", "settings:linkdl"), btn("🔥 ʟɪɴᴋᴅʟ ᴇғғᴇᴄᴛ", "settings:linkdleffect")],
...
def effect_menu(enabled: bool, current: str, prefix: str = "effect") -> InlineKeyboardMarkup:
    ...
    for key, label in EFFECT_LABELS.items():
        ...
        row.append(btn(f"{label}{tag}", f"{prefix}:set:{key}"))
        if len(row) == 2:
            rows.append(row)
            row = []
    if row:
        rows.append(row)
    rows.append([btn(toggle_label, f"{prefix}:toggle", toggle_style)])
    rows.append([back_btn()])
    return InlineKeyboardMarkup(rows)
