...
async def send_welcome_media(message: Message, welcome: dict, text: str) -> Message:
    ...
    if media_type == "video" and welcome.get("video_file_id"):
        return await message.reply_video(welcome["video_file_id"], caption=text, has_spoiler=spoiler)
    if media_type == "animation" and welcome.get("animation_file_id"):
        return await message.reply_animation(welcome["animation_file_id"], caption=text, has_spoiler=spoiler)
    if media_type == "photo" and welcome.get("photo_file_id"):
        return await message.reply_photo(welcome["photo_file_id"], caption=text, has_spoiler=spoiler)
    return await message.reply_text(text, link_preview_options=LinkPreviewOptions(is_disabled=True))
...
async def _send_customizable_popup(client, message: Message, popup_config: dict,
    ...
    if photo_id:
        await client.send_photo(
            message.chat.id, photo_id, caption=text,
