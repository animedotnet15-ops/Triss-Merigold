...
from triss.utils.keyboards import (
    ...
    effects_main_menu, effect_category_menu, linkdl_menu,
...
_ADMIN_COMMANDS = ["genlink", "batch", "done", "cancelbatch", "broadcast", "settings", "start", "addadmin", "removeadmin", "listadmin", "mute", "unmute", "listmute", "ban", "unban", "listban", "linkdl", "setlinkcaption"]
...
async def settings_router(client, cq: CallbackQuery) -> None:
    ...
    if action == "main":
        ...
        await _edit(
            cq, "🎬 ᴇғғᴇᴄᴛs\n\n"
                "⚡ Start · 🚨 Bypass · 🎉 Delivery · 🔥 Direct Download\n\n"
                "Open a category to pick any number of emoji effects for it "
                "(one is chosen at random each time). The 🔘 button below "
                "turns every category on/off at once without losing your picks.",
            effects_main_menu(settings.get("effects", {})),
        )
    elif action == "linkdl":
        ...
        await _edit(
            ...
            linkdl_menu(bool(ld.get("enabled")), has_url),
        )
    await cq.answer()
...
# ---------------------------------------------------------------------------
# 🎬 Effects (item 7 consolidated) — one master on/off, plus a per-
# category multi-select emoji picker for start / bypass / delivery /
# linkdl (see triss.database.mongodb DEFAULT_SETTINGS -> "effects").
# ---------------------------------------------------------------------------

@app.on_callback_query(filters.regex(r"^eff:"))
async def effects_router(client, cq: CallbackQuery) -> None:
    if await deny_if_not_owner(cq):
        return
    parts = cq.data.split(":")  # eff:master  |  eff:<cat>:open|toggle  |  eff:<cat>:pick:<key>
    settings = await db.get_settings()
    effects_settings = settings.get("effects", {})

    if parts[1] == "master":
        new_value = not effects_settings.get("master_enabled", True)
        await db.update_settings({"effects.master_enabled": new_value})
        effects_settings["master_enabled"] = new_value
        await cq.answer(f"All effects {'enabled' if new_value else 'disabled'}.", show_alert=True)
        await _edit(cq, "🎬 ᴇғғᴇᴄᴛs", effects_main_menu(effects_settings))
        return

    category = parts[1]
    from triss.utils.effects import EFFECT_CATEGORIES
    if category not in EFFECT_CATEGORIES:
        await cq.answer()
        return
    title = EFFECT_CATEGORIES[category]
    cat_settings = effects_settings.get("categories", {}).get(category, {"enabled": False, "selected": []})
    sub_action = parts[2] if len(parts) > 2 else "open"

    if sub_action == "open":
        await _edit(cq, title, effect_category_menu(category, cat_settings))
        await cq.answer()
        return

    if sub_action == "toggle":
        new_value = not cat_settings.get("enabled", False)
        await db.update_settings({f"effects.categories.{category}.enabled": new_value})
        cat_settings = {**cat_settings, "enabled": new_value}
        await cq.answer(f"{title} {'enabled' if new_value else 'disabled'}.", show_alert=True)
        await _edit(cq, title, effect_category_menu(category, cat_settings))
        return
    ...
    if sub_action == "preview":
        ...
        import random
        selected = cat_settings.get("selected") or []
        if not selected:
            await cq.answer("Pick an emoji first, then preview it.", show_alert=True)
            return
        effect_id = resolve_effect_id(random.choice(selected))
        ...
        await cq.answer("Sent below 👇")
        return

    if sub_action == "pick" and len(parts) > 3:
        effect_key = parts[3]
        selected = list(cat_settings.get("selected") or [])
        if effect_key in selected:
            selected.remove(effect_key)
        else:
            selected.append(effect_key)
        patch = {f"effects.categories.{category}.selected": selected}
        # Picking an emoji turns the category on by itself - matches "click
        # Start, send an emoji, it's set" from the original ask. Selecting
        # your first emoji from OFF auto-enables the category; clearing
        # the last one back out auto-disables it again, so the on/off
        # button always reflects reality instead of silently staying OFF
        # after a pick (the most likely reason effects "didn't play").
        was_enabled = cat_settings.get("enabled", False)
        new_enabled = was_enabled
        if selected and not was_enabled:
            new_enabled = True
            patch[f"effects.categories.{category}.enabled"] = True
        elif not selected and was_enabled:
            new_enabled = False
            patch[f"effects.categories.{category}.enabled"] = False
        await db.update_settings(patch)
        cat_settings = {**cat_settings, "selected": selected, "enabled": new_enabled}
        await cq.answer(f"{'Added' if effect_key in selected else 'Removed'} {effect_key}.")
        await _edit(cq, title, effect_category_menu(category, cat_settings))
        return
...
async def linkdl_router(client, cq: CallbackQuery) -> None:
    ...
    if action == "toggle":
        ...
        settings = await db.get_settings()
        await _edit(cq, "⬇️ Dɪʀᴇᴄᴛ Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋs (/linkdl)",
                    linkdl_menu(new_value, config.public_base_url is not None))
        return
    elif action == "settext":
        ...
        ld = settings.get("linkdl", {})
        await _edit(cq, "⬇️ Dɪʀᴇᴄᴛ Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋs (/linkdl)",
                    linkdl_menu(bool(ld.get("enabled")), config.public_base_url is not None))
        return
    await cq.answer()
...
def _register_popup_capture_handlers() -> None:
    for _popup_kind, (_field_name, _default_text, _title) in _POPUP_KINDS.items():
        ...
        async def _capture_popup_text(client, message: Message, field_name=_field_name) -> None:
            session_manager.clear(message.from_user.id)
            await db.update_settings({f"shortener.{field_name}.text": message.text.html})
        ...
        async def _capture_popup_photo(client, message: Message, field_name=_field_name) -> None:
            ...
            if message.caption:
                patch[f"shortener.{field_name}.text"] = message.caption.html
            await db.update_settings(patch)
