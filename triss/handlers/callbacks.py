...
async def settings_router(client, cq: CallbackQuery) -> None:
    ...
    if action == "main":
        ...
        de = settings.get("delivery_effect", {})
        await _edit(
            cq, "🎉 ᴅᴇʟɪᴠᴇʀʏ ᴇғғᴇᴄᴛ\n\nPlays a Telegram animation on the last delivered file "
                "message (private chats only, per Telegram's own restriction).",
            effect_menu(bool(de.get("enabled")), de.get("effect", "fire")),
        )
    elif action == "starteffect":
        se = settings.get("start_effect", {})
        await _edit(
            cq, "⚡ sᴛᴀʀᴛ ᴇғғᴇᴄᴛ\n\nPlays a Telegram animation on the final /start welcome message "
                "(private chats only).",
            effect_menu(bool(se.get("enabled")), se.get("effect", "party"), prefix="starteffect"),
        )
    elif action == "bypasseffect":
        be = settings.get("bypass_effect", {})
        await _edit(
            cq, "🚨 ʙʏᴘᴀss ᴇғғᴇᴄᴛ\n\nPlays a Telegram animation on the shortener's \"bypass detected\" "
                "popup (private chats only).",
            effect_menu(bool(be.get("enabled")), be.get("effect", "poop"), prefix="bypasseffect"),
        )
    elif action == "linkdleffect":
        le = settings.get("linkdl_effect", {})
        await _edit(
            cq, "🔥 ʟɪɴᴋᴅʟ ᴇғғᴇᴄᴛ\n\nPlays a Telegram animation on the /linkdl direct-download reply "
                "(private chats only).",
            effect_menu(bool(le.get("enabled")), le.get("effect", "fire"), prefix="linkdleffect"),
        )
    elif action == "linkdl":
        ...
        await _edit(
            cq, f"⬇️ Dɪʀᴇᴄᴛ Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋs (/linkdl)\n\n{custom}{status_note}",
...
# ---------------------------------------------------------------------------
# Delivery effect submenu (item 7)
# ---------------------------------------------------------------------------

@app.on_callback_query(filters.regex(r"^effect:"))
async def effect_router(client, cq: CallbackQuery) -> None:
    if await deny_if_not_owner(cq):
        return
    action = cq.data.split(":", 1)[1]
    settings = await db.get_settings()
    ...
    if action == "toggle":
        new_value = not de.get("enabled", False)
        await db.update_settings({"delivery_effect.enabled": new_value})
        await cq.answer(f"Delivery effect {'enabled' if new_value else 'disabled'}.", show_alert=True)
        await _edit(cq, "🎉 ᴅᴇʟɪᴠᴇʀʏ ᴇғғᴇᴄᴛ", effect_menu(new_value, de.get("effect", "fire")))
        return
    elif action.startswith("set:"):
        effect_key = action.split(":", 1)[1]
        await db.update_settings({"delivery_effect.effect": effect_key})
        await cq.answer(f"Effect set to {effect_key}.", show_alert=True)
        await _edit(cq, "🎉 ᴅᴇʟɪᴠᴇʀʏ ᴇғғᴇᴄᴛ", effect_menu(bool(de.get("enabled")), effect_key))
        return
    await cq.answer()


# ---------------------------------------------------------------------------
# Start / bypass-detected / linkdl effect submenus (same shape as the
# delivery effect above, three separate settings keys + send points)
# ---------------------------------------------------------------------------

_EXTRA_EFFECT_SETTINGS = {
    "starteffect": ("start_effect", "⚡ sᴛᴀʀᴛ ᴇғғᴇᴄᴛ"),
    "bypasseffect": ("bypass_effect", "🚨 ʙʏᴘᴀss ᴇғғᴇᴄᴛ"),
    "linkdleffect": ("linkdl_effect", "🔥 ʟɪɴᴋᴅʟ ᴇғғᴇᴄᴛ"),
}


@app.on_callback_query(filters.regex(r"^(starteffect|bypasseffect|linkdleffect):"))
async def extra_effect_router(client, cq: CallbackQuery) -> None:
    if await deny_if_not_owner(cq):
        return
    prefix, action = cq.data.split(":", 1)
    setting_key, title = _EXTRA_EFFECT_SETTINGS[prefix]
    settings = await db.get_settings()
    current = settings.get(setting_key, {})

    if action == "toggle":
        new_value = not current.get("enabled", False)
        await db.update_settings({f"{setting_key}.enabled": new_value})
        await cq.answer(f"{title} {'enabled' if new_value else 'disabled'}.", show_alert=True)
        await _edit(cq, title, effect_menu(new_value, current.get("effect", "fire"), prefix=prefix))
        return
    elif action.startswith("set:"):
        effect_key = action.split(":", 1)[1]
        await db.update_settings({f"{setting_key}.effect": effect_key})
        await cq.answer(f"Effect set to {effect_key}.", show_alert=True)
        await _edit(cq, title, effect_menu(bool(current.get("enabled")), effect_key, prefix=prefix))
        return
    await cq.answer()
