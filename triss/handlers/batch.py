...
from triss.services.logging_service import log_link_created_from_ref
...
async def done_command(client, message: Message) -> None:
    ...
    first = messages[0]
    await log_link_created_from_ref(
        client, user_id, message.from_user.username, link,
        first["chat_id"], first["message_id"], item_count=len(messages),
    )
