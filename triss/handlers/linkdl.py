...
def _tiny_placeholder_png(size: int = 32, rgb: tuple[int, int, int] = (37, 99, 235)) -> bytes:
    """Builds a minimal, valid, solid-colour PNG in pure stdlib (struct +
    zlib) - no image library dependency needed. Used only so the /linkdl
    reply can go out via send_photo instead of send_message: this
    installed Kurigram build's send_message() genuinely does not accept
    message_effect_id (confirmed via Render logs - TypeError naming the
    exact parameter, not a shortcut-forwarding issue like the earlier
    welcome-message bug), while send_photo does. Sent once ever per
    deployment (see _get_placeholder_file_id) and reused by file_id
    after that - never regenerated per message."""
...
async def _get_placeholder_file_id(client) -> str | None:
    """Returns a cached Telegram file_id for the tiny placeholder image,
    generating and uploading it to LOG_CHANNEL_ID exactly once (first
    /linkdl use ever) and persisting the file_id in settings.linkdl.
    placeholder_file_id from then on - every later /linkdl reply reuses
    that same file_id (near-instant, no re-upload)."""
    settings = await db.get_settings()
    cached = settings.get("linkdl", {}).get("placeholder_file_id")
    if cached:
        return cached
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as f:
            f.write(_tiny_placeholder_png())
            temp_path = f.name
        sent = await client.send_photo(config.log_channel_id, temp_path)
        file_id = sent.photo.file_id
        await db.update_settings({"linkdl.placeholder_file_id": file_id})
        return file_id
    except Exception:
        logger.warning("Could not generate/upload the linkdl placeholder image.", exc_info=True)
        return None
...
_EXCLUDED_COMMANDS = [
    ...
    "mute", "unmute", "listmute", "ban", "unban", "listban",
]
...
async def linkdl_capture(client, message: Message) -> None:
    ...
    user = message.from_user
    await log_linkdl_files(client, user.id, user.username, user.first_name)
    ...
    effect_id = pick_category_effect_id(settings, "linkdl")
    media_type = linkdl.get("media_type", "none")
    spoiler = bool(linkdl.get("spoiler"))
    btn_markup = InlineKeyboardMarkup([[url_btn("⬇️ Download", download_url)]])

    # Owner-configured media (photo/video/gif) takes priority over the
    # auto-generated placeholder - if they've set one, that's what they
    # want shown, effect or not.
    if media_type == "video" and linkdl.get("video_file_id"):
        await call_with_optional_effect(
            client.send_video, chat_id=message.chat.id, video=linkdl["video_file_id"],
            caption=caption, has_spoiler=spoiler, reply_markup=btn_markup, message_effect_id=effect_id,
        )
    elif media_type == "animation" and linkdl.get("animation_file_id"):
        await call_with_optional_effect(
            client.send_animation, chat_id=message.chat.id, animation=linkdl["animation_file_id"],
            caption=caption, has_spoiler=spoiler, reply_markup=btn_markup, message_effect_id=effect_id,
        )
    elif media_type == "photo" and linkdl.get("photo_file_id"):
        await call_with_optional_effect(
            client.send_photo, chat_id=message.chat.id, photo=linkdl["photo_file_id"],
            caption=caption, has_spoiler=spoiler, reply_markup=btn_markup, message_effect_id=effect_id,
        )
    else:
        # No media configured - fall back to the auto-generated
        # placeholder ONLY when an effect is actually selected (that's
        # the one reason a plain-text reply needs to become a photo at
        # all - see _get_placeholder_file_id's docstring). No effect
        # selected -> stays exactly as the original plain-text reply.
        placeholder = await _get_placeholder_file_id(client) if effect_id is not None else None
        if placeholder:
            await call_with_optional_effect(
                client.send_photo, chat_id=message.chat.id, photo=placeholder,
                caption=caption, reply_markup=btn_markup, message_effect_id=effect_id,
            )
        else:
            await call_with_optional_effect(
                client.send_message, chat_id=message.chat.id, text=caption,
                reply_markup=btn_markup, message_effect_id=effect_id,
            )
