...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    "linkdl": {"enabled": False, "caption": None, "placeholder_file_id": None},
    ...
    # Item 7: Telegram's built-in animated message effects (🎉/🔥/etc,
    # see triss.utils.effects), played at 4 different send points. One
    # master on/off switch, plus one on/off + a MULTI-select emoji list
    # per category (Telegram allows only one effect per message, so when
    # several emojis are enabled for a category the bot picks one at
    # random each time - see effects.pick_category_effect_id).
    "effects": {
        "master_enabled": True,
        "categories": {
            "start": {"enabled": False, "selected": []},
            "bypass": {"enabled": False, "selected": []},
            "delivery": {"enabled": False, "selected": []},
            "linkdl": {"enabled": False, "selected": []},
        },
    },
