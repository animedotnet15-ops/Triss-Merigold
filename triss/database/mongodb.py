...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    # The three effect toggles the owner asked for alongside the existing
    # delivery effect above - same shape ({enabled, effect}), same 6
    # standard Telegram effects (see triss.utils.effects), applied at
    # three different send points:
    "start_effect": {"enabled": False, "effect": "party"},       # ⚡ on the final /start welcome message
    "bypass_effect": {"enabled": False, "effect": "poop"},        # 🚨 on the shortener "bypass detected" popup
    "linkdl_effect": {"enabled": False, "effect": "fire"},        # 🔥 on the /linkdl direct-download reply
